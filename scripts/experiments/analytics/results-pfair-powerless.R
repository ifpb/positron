  ...
  for (policy in policies) {
    for (scenario in scenarios) {
      for (turn in turns) {
        ...
        filename <- paste("../results/JISA/powerless/", lossScenario, "/pfair-", scenario, "nodes-", policy, "-", turn, ".csv", sep = "")
...
for (lossScenario in lossScenarios) {
  ...
  gplotPfair <- ggplot(dataAvg[dataAvg$loss == lossScenario,], 
  ...
  ggsave(file=paste("pfair-powerless-", lossScenario, ".pdf", sep=""), height=7.0, width=10.0, units="in")
