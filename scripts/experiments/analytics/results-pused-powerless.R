...
# loss <- readline(prompt="\nEnter power scenario (powerfull|powerless): ")
# if ( loss != "powerfull" & loss != "powerless" ) {
...
colnames(data) <- c("power", "loss", "policies", "nodes",  "turns", "values")
auxBalDF <- data.frame( matrix(ncol = 6, nrow = 0) )
colnames(auxBalDF) <- c("power", "loss", "policies", "nodes",  "turns", "values")
auxSatDF <- data.frame( matrix(ncol = 6, nrow = 0) )
...
for (lossScenario in lossScenarios) {
  ...
  for (scenario in scenarios) {
    ...
    filename <- paste("../results/JISA/powerless/", lossScenario, "/pused-", scenario, "nodes.txt", sep = "")
    auxFile <- read.csv(filename)

    auxDF <- data.frame(
      power=rep(powerScenario, length(auxFile$puBal)),
      loss=rep(lossScenario, length(auxFile$puBal)),
    ...
    auxDF <- data.frame(
      power=rep(powerScenario, length(auxFile$puSat)),
      loss=rep(lossScenario, length(auxFile$puSat)),
...
dataAvg <- data.frame( matrix(ncol = 7, nrow = 0) )
colnames(dataAvg) <- c("power", "loss", "policies", "nodes", "upper", "mean", "lower")
auxBalAvg <- data.frame( matrix(ncol = 7, nrow = 0) )
colnames(auxBalAvg) <- c("power", "loss", "policies", "nodes", "upper", "mean", "lower")
auxSatAvg <- data.frame( matrix(ncol = 7, nrow = 0) )
colnames(auxSatAvg) <- c("power", "loss", "policies", "nodes", "upper", "mean", "lower")

for (lossScenario in lossScenarios) {
  for (scenario in scenarios) {
      
      auxVector <- Rmisc::CI(
        data[data$power == powerScenario & data$loss == lossScenario & data$policies == "bal" & data$nodes == scenario,]$values
        )
      auxDF <- data.frame(
        power=powerScenario,
        loss=lossScenario,
      ...
      auxVector <- Rmisc::CI(
        data[data$power == powerScenario & data$loss == lossScenario & data$policies == "sat" & data$nodes == scenario,]$values
      )
      auxDF <- data.frame(
        power=powerScenario,
        loss=lossScenario,
...
for (lossScenario in lossScenarios) {
  ...
  gplotPused <- ggplot(dataAvg[dataAvg$loss == lossScenario,], 
  ...
  ggsave(file=paste("pused-powerless-", lossScenario, ".pdf", sep=""), height=5.0, width=10.0, units="in")

}

rm(auxDF, auxBalAvg, auxSatAvg, auxVector, scenario, scenarios, powerScenario, lossScenario, lossScenarios, gplotPused)