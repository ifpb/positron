...
for (policy in policies) {
  for (scenario in scenarios) {
    for (turn in turns) {
      ...
      filename <- paste("../results/JISA/powerfull/pefficient-", scenario, "nodes-", policy, "-", turn, ".csv", sep = "")
...
rm(auxFile, auxDF, filename, group, policy, scenario, turn)
  ...
  for (scenario in scenarios) {
    for (group in groups) {
      ...
      auxDF <- data.frame(
        ...
        groups=group,
        means=mean(
          data[data$policies == policy & data$nodes == scenario & data$groups == group,]$values
          )
      )
...
rm(auxDF, policy, scenario, group)

gplotPefficient <- ggplot(dataAvg, 
...
ggsave(file=paste("pefficient-powerfull.pdf", sep=""), height=7.0, width=10.0, units="in")
