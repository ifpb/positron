...
colnames(data) <- c("power", "policies", "nodes",  "turns", "values")
auxBalDF <- data.frame( matrix(ncol = 5, nrow = 0) )
colnames(auxBalDF) <- c("power", "policies", "nodes",  "turns", "values")
...
colnames(dataAvg) <- c("power", "policies", "nodes", "upper", "mean", "lower")
auxBalAvg <- data.frame( matrix(ncol = 6, nrow = 0) )
colnames(auxBalAvg) <- c("power", "policies", "nodes", "upper", "mean", "lower")
auxSatAvg <- data.frame( matrix(ncol = 6, nrow = 0) )
colnames(auxSatAvg) <- c("power", "policies", "nodes", "upper", "mean", "lower")

for (powerScenario in powerScenarios) {
  for (scenario in scenarios) {
      
      auxVector <- Rmisc::CI(
        data[data$power == powerScenario & data$policies == "bal" & data$nodes == scenario,]$values
        )
      auxDF <- data.frame(
        power=powerScenario,
        policies="bal", 
      ...
      auxVector <- Rmisc::CI(
        data[data$power == powerScenario & data$policies == "sat" & data$nodes == scenario,]$values
      )
      auxDF <- data.frame(
        power=powerScenario,
        policies="sat", 
...
ggsave(file=paste("pused-powerALL.pdf", sep=""), height=7.0, width=10.0, units="in")