rm(list = ls())

# Packages needed
packages_needed <- c("ggplot2", "Rmisc", "tidyverse", "colorspace", "cowplot",
                     "rcartocolor", "ggforce", "ggdist", "ggridges", "ggimage",
                     "ggbeeswarm", "gghalves", "systemfonts", "grid")

# Install packages not yet installed
packages_installed <- packages_needed %in% rownames(installed.packages())
if (any(packages_installed == FALSE)) {
  print(paste(packages_needed[!packages_installed], " not installed!!", sep = " "))
  quit()
} 

packages_loaded <- packages_needed %in% .packages()
if (any(packages_loaded == FALSE)) {
  invisible(
    lapply(
      packages_needed[!packages_loaded], library, character.only = TRUE
    )
  )
}
rm(packages_installed, packages_loaded, packages_needed)

# Need a symbolic link to wherever folder it is located: $ ln -s /path/to/positron ~/positron
working_directory <- "~/positron/scripts/experiments/analytics"
if (getwd() != working_directory) setwd("~/positron/scripts/experiments/analytics")
rm(working_directory)

data <- data.frame( matrix(ncol = 6, nrow = 0) )
colnames(data) <- c("policies", "loss", "nodes",  "groups", "turns", "values")

powerScenario <- "powerless"
lossScenarios <- c("10", "20", "25", "30", "50")
# lossScenarios <- c("30e")
scenarios <- c("30", "60", "90", "120", "150", "180")
policies <- c("bal", "sat")
groups <- seq(1,4)
turns <- seq(1,30)

for (lossScenario in lossScenarios) {
  for (policy in policies) {
    for (scenario in scenarios) {
      for (turn in turns) {

        filename <- paste("../results/JISA/powerless/", lossScenario, "/pefficient-", scenario, "nodes-", policy, "-", turn, ".csv", sep = "")
        auxFile <- read.csv(filename)

        for (group in groups) {

          auxDF <- data.frame(
            policies=policy,
            loss=lossScenario, 
            nodes=scenario,
            groups=group,
            turns=turn, 
            values=ifelse(is.na(auxFile$pefficient[group]), 0, auxFile$pefficient[group])
            )
          data <- rbind(data, auxDF)
        
        }
      }
    }
  }
}

rm(auxFile, auxDF, filename, group, policy, scenario, turn, lossScenario)

dataAvg <- data.frame( matrix(ncol = 5, nrow = 0) )
colnames(dataAvg) <- c("policies", "loss", "nodes", "groups", "means")

for (lossScenario in lossScenarios) {
  for (policy in policies) {
    for (scenario in scenarios) {
      for (group in groups) {
        
        auxDF <- data.frame(
          policies=policy,
          loss=lossScenario, 
          nodes=scenario,
          groups=group,
          means=mean(
            data[data$policies == policy & data$loss == lossScenario & data$nodes == scenario & data$groups == group,]$values
            )
        )

        dataAvg <- rbind(dataAvg, auxDF)

      }
    }
  }
}

rm(auxDF, policy, scenario, group, lossScenario)

for (lossScenario in lossScenarios) {

  gplotPefficient <- ggplot(dataAvg[dataAvg$loss == lossScenario,], 
                      aes(x = factor(nodes, level = scenarios),
                          y=means,
                          fill=factor(-groups),
                          label = round(means, digits = 1)
                          )
                      ) +
    theme_bw() +
    geom_bar(stat = "identity", alpha=0.5) +
    geom_text(position = position_stack(vjust = 0.5), size = 7, fontface = "bold") +
    scale_fill_manual(name = "Allocated groups:", 
                      labels = c("Inefficiently", "Group 1", "Group 2", "Group 3"),
                      values = c("red", "gray","#E69F00","#56B4E9")
                      ) +
    labs(title = "", x = "Total amount of Workers", 
        y =  "% of efficient allocation") +
    # coord_flip() +
    facet_grid(~policies, 
              labeller = labeller(
                policies = c(bal = "Balanced", sat = "Saturation")
                )
              ) +
    theme(
      legend.position = "top",
      legend.text = element_text(face = "bold", size = 20),
      legend.title = element_text(face = "bold", size = 20),
      axis.text = element_text(face = "bold", size = 20),
      axis.title = element_text(face = "bold", size = 20),
      strip.text = element_text(face = "bold", size = 20)
    )

  t_col <- function(color, percent = 50, name = NULL) {
        #      color = color name
    #    percent = % transparency
    #       name = an optional name for the color

    ## Get RGB values for named color
    rgb.val <- col2rgb(color)

    ## Make new color using input color as base and alpha set by transparency
    t.col <- rgb(rgb.val[1], rgb.val[2], rgb.val[3],
              max = 255,
              alpha = (100 - percent) * 255 / 100,
              names = name
              )

    ## Save the color
    invisible(t.col)
  }

  bal.col <- t_col("#FB9A99")
  sat.col <- t_col("#CAB2D6")
  # fills <- c("#FB9A99", "#CAB2D6")
  fills <- c(bal.col, sat.col)

  gplotAux <- ggplot_gtable(ggplot_build(gplotPefficient))
  striprt <- which( grepl('strip-r', gplotAux$layout$name) | grepl('strip-t', gplotAux$layout$name) )
  k <- 1
  for (i in striprt) {
    j <- which(grepl('rect', gplotAux$grobs[[i]]$grobs[[1]]$childrenOrder))
    gplotAux$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- fills[k]
    k <- k + 1
  }
  gplotPefficient <- plot_grid(gplotAux)

  ggsave(file=paste("pefficient-powerless-", lossScenario, ".pdf", sep=""), height=7.0, width=10.0, units="in")

}