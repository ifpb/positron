...
for (lossScenario in lossScenarios) {
  for (scenario in scenarios) {
    ...
    filename <- paste("../results/JISA/powerless/", lossScenario, "/ppreempted-", scenario, "nodes.txt", sep = "")
    auxFile <- read.csv(filename)

    auxDF <- data.frame(
      policies=rep("bal", length(auxFile$ppBal)), 
      loss=rep(lossScenario, length(auxFile$ppBal)),
    ...
    auxDF <- data.frame(
      ...
      loss=rep(lossScenario, length(auxFile$ppSat)), 
...
for (lossScenario in lossScenarios) {
  ...
  gplotPpreempted <- ggplot(dataAvg[dataAvg$loss == lossScenario,], 
  ...
  ggsave(file=paste("ppreempted-powerless-", lossScenario, ".pdf", sep=""), height=5.0, width=10.0, units="in")
