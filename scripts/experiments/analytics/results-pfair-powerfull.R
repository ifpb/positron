...
for (policy in policies) {
  for (scenario in scenarios) {
    for (turn in turns) {
      ...
      filename <- paste("../results/JISA/powerfull/pfair-", scenario, "nodes-", policy, "-", turn, ".csv", sep = "")
...
ggsave(file=paste("pfair-powerfull.pdf", sep=""), height=7.0, width=10.0, units="in")
